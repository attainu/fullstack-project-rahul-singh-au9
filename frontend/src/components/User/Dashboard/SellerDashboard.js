...
const SellerDashboard = () => {
    ...
    const fetchSellerServices = async () => {
        let {data} = await sellerServices(userInfo.token);
        setServices(data)
    }
    ...
    const handleClick = async () => {
        setLoading(true);
        try {
        let res = await createStripeAccount(userInfo.token);
    ...
    const handleServiceDelete = async (serviceId) => {
        ...
        deleteService(userInfo.token, serviceId).then((res) =>{
    ...
    return (
        <>
        {
            userInfo && userInfo.stripe_seller &&
            userInfo.stripe_seller.charges_enabled ? connected() : notConnected()
        }
